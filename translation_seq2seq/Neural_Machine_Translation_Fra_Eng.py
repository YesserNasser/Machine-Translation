...
raw_data = open('fra.txt',encoding='UTF-8').read().strip().split('\n')
num_examples = len(raw_data)
lan1_lan2 = [[preprocess_sentence(s) for s in l.split('\t')] for l in raw_data[:num_examples]]

# loading data
inp_lang = LangIndex(fr for en, fr in lan1_lan2)
targ_lang = LangIndex(en for en, fr in lan1_lan2)

# Vectorize the input and target languages
# Spanish sentences
input_tensor = [[inp_lang.word2idx[s] for s in fr.split(' ')] for en, fr in lan1_lan2]
    
# English sentences
target_tensor = [[targ_lang.word2idx[s] for s in en.split(' ')] for en, fr in lan1_lan2]
...
translate('Un vrai scientifique ne penserait pas de cette manière.', encoder, decoder, inp_lang, targ_lang, max_length_inp, max_length_targ)
